...
export const authService = {
  ...
  signUp: async (email, password, name) => {
    if (supabase) {
      ...
      const newUser = { email, password, name, downloads: [] };
  ...
  signIn: async (email, password) => {
    if (supabase) {
      ...
      localStorage.setItem('designali_mock_session', JSON.stringify(user));
      return user;
    }
  },
  ...
  getCurrentUser: async () => {
    if (supabase) {
      ...
      try {
        return JSON.parse(localStorage.getItem('designali_mock_session'));
      } catch (e) {
        return null;
      }
    }
